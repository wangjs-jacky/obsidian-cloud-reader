...
test('saved credentials are returned only by explicit reveal POST, never ordinary settings',async()=>{const f=fixture();f.l.testSource={config:{endpoint:'oss-cn-shanghai.aliyuncs.com',bucket:'test-bucket',region:'cn-shanghai',prefix:'',accessKeyId:'test-id',secretAccessKey:'test-secret'}};const call=(path,method='GET')=>f.l.fetch(new Request('https://reader.test'+path,{method}));const normal=await(await call('/api/settings')).json();assert.equal(normal.accessKeyId,undefined);assert.equal(normal.secretAccessKey,undefined);assert.equal((await call('/api/settings/reveal')).status,405);assert.deepEqual(await(await call('/api/settings/reveal','POST')).json(),{accessKeyId:'test-id',secretAccessKey:'test-secret'});f.l.testSource=null;assert.equal((await call('/api/settings/reveal','POST')).status,409)})
